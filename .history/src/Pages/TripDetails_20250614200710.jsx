...
function TripDetails() {
  ...
  return (
    <div className="trip-container">
      ...
      <div className="flex flex-column justify-content-between align-items-center mt-3 gap-2">
      ...
      {trip.isFinished && hotelReviews.length === 0 && (
        <div style={{ marginTop: 24 }}>
          <h3>Đánh giá Khách Sạn</h3>
          <div className="form-group">
            <label>Chọn số sao:</label>
            <div className="star-rating" style={{ fontSize: 28, color: '#ffc107', cursor: 'pointer' }}>
              {[1, 2, 3, 4, 5].map((star) => (
                <span
                  key={star}
                  onClick={() => setReview(prev => ({ ...prev, rating: star }))}
                  onMouseOver={() => setReview(prev => ({ ...prev, hover: star }))}
                  onMouseLeave={() => setReview(prev => ({ ...prev, hover: 0 }))}
                >
                  {review.hover >= star || review.rating >= star ? '★' : '☆'}
                </span>
              ))}
            </div>
          </div>
      ...
      {hotelReviews &&
        <>
          <div><strong>Đánh giá của tôi</strong></div>
          {hotelReviews.length > 0 && (
            <div className="mt-2">
              <ul className="list-group">
                {hotelReviews.map((r, i) => (
                  <li className="list-group-item" key={i}>
                    <div><strong>{r.userName}</strong> đánh giá {r.rating}⭐</div>
                    <div>{r.comment}</div>
