    ...
    <div className="container mt-4">
                  ...
                  <div className={`card h-100 ${isSelected ? 'border-success shadow' : ''}`}>
                    ...
                    <div className="card-body">
                      <label style={{ cursor: 'pointer' }}>
                        ...
                        />
                        {/* Mô tả room.name phải thật nổi bật và đẹp */}
                        <h5 className="card-title" style={{ fontSize: '20px', fontWeight: 'bold' }}>
                          {room.name} {room.discount > 0 && <span className="badge bg-danger ms-2">-{room.discount* 100}%</span>}
                        </h5>
                          ...
                          <span className="text-danger" style={{ fontSize: '24px' }}>
                           ...
                           {room.discount > 0 && (
                              <span className="text-decoration-line-through me-2" style={{ color: '#6c757d' }}>
                                {/* Đơn vị tiền tệ là vietnam dong, ví dụ 1.234.544 đ */}
                                {room.pricePerNight?.toFixed(2)} đ
                              </span>
                            )}
                            {room.discount > 0 && (
                              <span className="text-danger ms-2" style={{ color: '#6c757d' }}>
                                {(room.pricePerNight * (1 - room.discount / 100))?.toFixed(2)} đ
                              </span>
                            )}
                          </span>
                     
                        {room.description2 && <p className="mt-2" style={{fontSize: 14, fontWeight: 400}}>- {room.description2}</p>}
                        {room.description3 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description3}</p>}
                        {room.description4 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description4}</p>}
                        {room.description5 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description5}</p>}
                        {room.description6 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description6}</p>}
                        {room.description7 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description7}</p>}
                        {room.description8 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description8}</p>}
                        {room.description9 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description9}</p>}
                        {room.description10 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description10}</p>}
                        {room.description11 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description11}</p>}
                        {room.description12 && <p className="mt-2"  style={{fontSize: 14, fontWeight: 400}}>-{room.description12}</p>}
                      </label>
      ...
      <div className="mt-4"></div>
        <button
          ...
          disabled={!selectedHotelId || roomBookings.length === 0}
